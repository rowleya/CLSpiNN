...
for i, (pipe, chip_coords, _, _) in enumerate(DEVICE_PARAMETERS):
    ...
    # This makes use of spif using the following:
    ...
    # This tells the machine to configure a SPIF devices as follows:
    # pipe: Which spif pipe to use on the SPIF board in question
    # width, hight: The width and height of the retina in pixels
    # sub_width, sub_height: How to split the retina up, to avoid sending all pixels everywhere.  In particular, if you are sending to a convolution or other 2D population, this will reduce the input load on that population.
    # input_x_shift, input_y_shift: The shift to apply to the input received from the Ethernet packets received by spif.  I don’t know how this applies to USB, but it will be how the X and Y coordinates are to be extracted from a received message.
    # chip_coords: Which chip of the machine the spif board you want to use is connected to as a tuple of (x, y).
    ...
    # To make a 2D population in PyNN, you need a structure parameter e.g.:
    # capture = p.Population(
    #         WIDTH * HEIGHT, p.IF_curr_exp(), structure=Grid2D(WIDTH / HEIGHT),
    #         label=f"Capture for device {i}")
    
    # A normal projection should then connect the two in the optimum way.
    ...
    print("\n\n\n\nRecording spikes for capture population")
